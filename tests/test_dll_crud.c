...
int test_insert_front_back_find(void)
{
    ...
    *c = 3;
    ASSERT_EQ(dsc_dll_push_front(list, a), 0);
    ASSERT_EQ(dsc_dll_push_back(list, b), 0);
    ASSERT_EQ(dsc_dll_push_back(list, c), 0);
...
int test_remove(void)
{
    ...
    *c = 3;
    ...
    const int key = 2;
    ASSERT_EQ(dsc_dll_remove(list, &key, int_cmp, true), 0);
...
int test_remove_not_found(void)
{
    ...
    *a = 1;
    ...
    const int key = 99;
    ASSERT_EQ(dsc_dll_remove(list, &key, int_cmp, true), -1);
...
{
    ASSERT_EQ(dsc_dll_push_back(NULL, NULL), -1);
    ASSERT_EQ(dsc_dll_push_front(NULL, NULL), -1);
    ASSERT_EQ_PTR(dsc_dll_find(NULL, NULL, NULL), NULL);
    ASSERT_EQ(dsc_dll_remove(NULL, NULL, NULL, false), -1);
    dsc_dll_destroy(NULL, false); // Should not crash
    return TEST_SUCCESS;
}
...
int test_insert_at(void)
{
    ...
    *b = 2;
    int* c = malloc(sizeof(int));
    *c = 3;
    ASSERT_EQ(dsc_dll_push_back(list, a), 0);  // [1]
    ASSERT_EQ(dsc_dll_push_back(list, c), 0);  // [1,3]
...
int test_remove_at(void)
{
    ...
    *b = 20;
    int* c = malloc(sizeof(int));
    *c = 30;
    dsc_dll_push_back(list, a); // [10]
    dsc_dll_push_back(list, b); // [10,20]
    dsc_dll_push_back(list, c); // [10,20,30]
...
int test_remove_front(void)
{
    ...
    // Test on empty list
    ASSERT_EQ(dsc_dll_pop_front(list, true), -1);
    ...
    *c = 30;
    ...
    int key = 10;
    ASSERT_NULL(dsc_dll_find(list, &key, int_cmp));
    key = 20;
    ASSERT_NOT_NULL(dsc_dll_find(list, &key, int_cmp));
    ASSERT_EQ(list->head->data, dsc_dll_find(list, &key, int_cmp)->data);
    ASSERT_NULL(list->head->prev);
...
int test_remove_back(void)
{
    ...
    // Test on empty list
    ASSERT_EQ(dsc_dll_pop_back(list, true), -1);

    // Test on single element list
    int* a = malloc(sizeof(int));
    *a = 10;
    dsc_dll_push_back(list, a);
    ASSERT_EQ(dsc_dll_pop_back(list, true), 0);
    ASSERT_EQ(list->size, 0);
    ASSERT_NULL(list->head);
    ASSERT_NULL(list->tail);
    ...
    *d = 40;
    ...
    key = 30;
    ASSERT_NOT_NULL(dsc_dll_find(list, &key, int_cmp));
    ASSERT_EQ(list->tail->data, dsc_dll_find(list, &key, int_cmp)->data);
    ASSERT_NULL(list->tail->next);
...
int test_remove_at_head(void)
{
    ...
    *a = 100;
    int* b = malloc(sizeof(int));
    *b = 200;
    dsc_dll_push_back(list, a); // [100]
    dsc_dll_push_back(list, b); // [100,200]

    ASSERT_EQ(dsc_dll_remove_at(list, 0, true), 0); // remove head (100)
...
int test_remove_at_last(void)
{
    ...
    *b = 2;
    int* c = malloc(sizeof(int));
    *c = 3;
    dsc_dll_push_back(list, a); // [1]
    dsc_dll_push_back(list, b); // [1,2]
    dsc_dll_push_back(list, c); // [1,2,3]

    ASSERT_EQ(dsc_dll_remove_at(list, 2, true), 0); // remove last (3)
    ASSERT_EQ(list->size, 2);
...
int test_remove_at_invalid(void)
{
    ...
    *a = 1;
    ...
    ASSERT_EQ(dsc_dll_remove_at(list, (size_t)-1, true), -1); // negative as size_t (very large)
...
int test_insert_remove_null_data(void)
{
    DSCAllocator alloc = create_int_allocator();
    DSCDoublyLinkedList* list = dsc_dll_create(&alloc);
    ASSERT_EQ(dsc_dll_push_back(list, NULL), 0); // allow NULL data
    ASSERT_EQ(list->size, 1);
    ASSERT_EQ(dsc_dll_remove_at(list, 0, false), 0); // remove node with NULL data, no free
    ASSERT_EQ(list->size, 0);
    dsc_dll_destroy(list, true);
    return TEST_SUCCESS;
}
...
int test_mixed_operations_integrity(void)
{
    ...
    *c = 30;
    dsc_dll_push_back(list, a);  // [10]
    dsc_dll_push_front(list, b); // [20,10]
...
int test_is_empty(void)
{
    ...
    *a = 10;
    ...
    dsc_dll_remove_at(list, 0, true);
    ASSERT_EQ(dsc_dll_is_empty(list), 1); // Empty again

    ASSERT_EQ(dsc_dll_is_empty(NULL), 1); // NULL list should be considered empty
...
int test_complex_data_type(void)
{
    ...
    const DSCDoublyLinkedNode* found = dsc_dll_find(list, &search_key, person_cmp);
    ASSERT_NOT_NULL(found);
...
int test_remove_all(void)
{
    ...
    while (!dsc_dll_is_empty(list))
    {
        dsc_dll_pop_front(list, true);
    }
    ...
    dsc_dll_destroy(list, false); // Already freed all data
    return TEST_SUCCESS;
}
