...
int test_bst_no_free_data(void)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ASSERT_EQ(dsc_bst_insert(bst, &static_values[i]), 0);
    }
...
int test_bst_remove_memory(void)
{
    ...
    ASSERT(!dsc_bst_contains(bst, &remove_val));

    // Remove without freeing data (for static reference)
    int static_val = 20;
    ASSERT_EQ(dsc_bst_remove(bst, &static_val, true), 0);
    ASSERT_EQ(dsc_bst_size(bst), 3);

    dsc_bst_destroy(bst, true); // Free remaining data
    return TEST_SUCCESS;
}
...
int test_bst_large_dataset(void)
{
    ...
    ASSERT_EQ(dsc_bst_size(bst), NUM_ELEMENTS);

    // Verify some elements exist
    for (int i = 0; i < NUM_ELEMENTS; i += 100)
    {
        ASSERT(dsc_bst_contains(bst, &i));
    }

    // Remove half the elements
    for (int i = 0; i < NUM_ELEMENTS; i += 2)
    {
        ASSERT_EQ(dsc_bst_remove(bst, &i, true), 0);
    }

    ASSERT_EQ(dsc_bst_size(bst), NUM_ELEMENTS / 2);
...
int test_bst_person_memory(void)
{
    ...
    DSCBinarySearchTree* bst = dsc_bst_create(&alloc, person_cmp);
    ...
    for (int i = 0; i < 3; i++)
    {
        ASSERT_EQ(dsc_bst_insert(bst, people[i]), 0);
    }

    ASSERT_EQ(dsc_bst_size(bst), 3);

    // Verify people are in the tree
    ASSERT(dsc_bst_contains(bst, people[0]));
    ASSERT(dsc_bst_contains(bst, people[1]));
    ASSERT(dsc_bst_contains(bst, people[2]));

    dsc_bst_destroy(bst, true); // Free person data
    return TEST_SUCCESS;
}
...
int test_bst_iterator_memory(void)
{
    ...
    DSCIterator it1 = dsc_bst_iterator(bst);
    DSCIterator it2 = dsc_bst_iterator_preorder(bst);
    DSCIterator it3 = dsc_bst_iterator_postorder(bst);
...
int test_bst_from_iterator_memory(void)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        int* data = malloc(sizeof(int));
        *data = i * 10;
        ASSERT_EQ(dsc_bst_insert(source_bst, data), 0);
    }

    // Test with copying data
    DSCIterator it1 = dsc_bst_iterator(source_bst);
    DSCBinarySearchTree* copy_bst = dsc_bst_from_iterator(&it1, &alloc, int_cmp, true);
    ASSERT_NOT_NULL(copy_bst);
    ASSERT_EQ(dsc_bst_size(copy_bst), 5);

    // Test without copying data
    it1.reset(&it1);
    DSCBinarySearchTree* ref_bst = dsc_bst_from_iterator(&it1, &alloc, int_cmp, false);
    ASSERT_NOT_NULL(ref_bst);
    ASSERT_EQ(dsc_bst_size(ref_bst), 5);

    it1.destroy(&it1);

    // Clean up - copy_bst should free its copied data, ref_bst should not
    dsc_bst_destroy(ref_bst, false);
    dsc_bst_destroy(copy_bst, true);
...
int test_bst_memory_edge_cases(void)
{
    ...
    *single_data = 42;
    ASSERT_EQ(dsc_bst_insert(bst, single_data), 0);
...
int test_bst_destruction_states(void)
{
    ...
    DSCBinarySearchTree* empty_bst = dsc_bst_create(&alloc, int_cmp);
    dsc_bst_destroy(empty_bst, true);

    // Test destroying single node tree
    DSCBinarySearchTree* single_bst = dsc_bst_create(&alloc, int_cmp);
    int* data = malloc(sizeof(int));
    *data = 42;
    dsc_bst_insert(single_bst, data);
    dsc_bst_destroy(single_bst, true);

    // Test destroying after clear
    DSCBinarySearchTree* cleared_bst = dsc_bst_create(&alloc, int_cmp);
    for (int i = 0; i < 5; i++)
    {
        int* val = malloc(sizeof(int));
        *val = i;
        dsc_bst_insert(cleared_bst, val);
    }
    dsc_bst_clear(cleared_bst, true);
    dsc_bst_destroy(cleared_bst, false); // Already cleared
