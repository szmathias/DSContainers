...
static int test_multiple_iterators(void) {
    ...
    // Cleanup
    if (it1.destroy) it1.destroy(&it1);
    if (it2.destroy) it2.destroy(&it2);
    dll_destroy(list, true);
    destroy_allocator(alloc);
    return TEST_SUCCESS;
}
...
static int test_iterator_reset(void) {
    ...
    for (int i = 1; i <= 3; i++) { int* val = malloc(sizeof(int)); *val = i; dll_insert_back(list, val); }

    Iterator it = dll_iterator(list);
    while (it.has_next(&it)) it.next(&it);
    ASSERT_FALSE(it.has_next(&it));
...
static int test_reverse_iteration(void) {
    ...
    for (int i = 1; i <= 5; i++) { int* val = malloc(sizeof(int)); *val = i; dll_insert_back(list, val); }
...
// Helper functions
static void* copy_int(const void* data) { if (!data) return NULL; int* copy = malloc(sizeof(int)); if (!copy) return NULL; *copy = *(const int*)data; return copy; }
static int int_compare(const void* a, const void* b) { const int* ia=a; const int* ib=b; if (*ia==*ib) return 0; return (*ia<*ib)?-1:1; }

// Test converting a basic iterator to a list with dll_from_iterator_custom
static int test_from_iterator_basic(void) {
    Alloc *src_alloc = create_std_allocator();
    DoublyLinkedList* src_list = dll_create(src_alloc);
    ASSERT_NOT_NULL(src_list);

    for (int i = 1; i <= 5; i++) { int* val = malloc(sizeof(int)); *val = i; dll_insert_back(src_list, val); }

    Iterator it = dll_iterator(src_list);
    ASSERT_TRUE(it.has_next(&it));

    // Convert iterator to a new list (shallow copy - same data pointers)
    DoublyLinkedList* new_list = dll_copy(src_list);
    ASSERT_NOT_NULL(new_list);
    ASSERT_EQ(dll_size(new_list), 5);

    Iterator new_it = dll_iterator(new_list);
    int expected = 1;
    while (new_it.has_next(&new_it)) { const int* value = new_it.next(&new_it); ASSERT_EQ(*value, expected++); }
    ...
    *((int*)node->data) = 99;
    ...
    if (new_it.destroy) new_it.destroy(&new_it);
    if (it.destroy) it.destroy(&it);

    dll_destroy(new_list, false); // Don't free data - shared with src_list
    dll_destroy(src_list, true); // Free the data
    destroy_allocator(src_alloc);

    return TEST_SUCCESS;
}

// Test deep copying with dll_from_iterator_custom using a copy function
static int test_from_iterator_with_copy(void) {
    Alloc *alloc = create_std_allocator();
    DoublyLinkedList* src_list = dll_create(alloc);
    ASSERT_NOT_NULL(src_list);
    ...
    Alloc *alloc_copy = create_std_allocator();
    alloc_copy->copy_func = copy_int;
    alloc_copy->data_free_func = free;
    DoublyLinkedList* new_list = dll_from_iterator(&it, alloc_copy);
    ASSERT_NOT_NULL(new_list);
    ASSERT_EQ(dll_size(new_list), 5);
    ...
    *((int*)node->data) = 99;
    ...
    Iterator new_it = dll_iterator(new_list);
    int expected2 = 1;
    while (new_it.has_next(&new_it)) { const int* value = new_it.next(&new_it); ASSERT_EQ(*value, expected2++); }

    if (new_it.destroy) new_it.destroy(&new_it);
    if (it.destroy) it.destroy(&it);

    dll_destroy(new_list, true);
    dll_destroy(src_list, true);
    destroy_allocator(alloc_copy);
    destroy_allocator(alloc);
    return TEST_SUCCESS;
}

// Test dll_from_iterator_custom with an empty list
static int test_from_iterator_empty(void) {
    ...
    DoublyLinkedList* new_list = dll_from_iterator(&it, alloc);
    ASSERT_NOT_NULL(new_list);
    ASSERT_EQ(dll_size(new_list), 0);
    ASSERT_TRUE(dll_is_empty(new_list));

    if (it.destroy) it.destroy(&it);
    dll_destroy(new_list, false);
    dll_destroy(src_list, false);
    destroy_allocator(alloc);
    return TEST_SUCCESS;
}

// Test dll_from_iterator_custom with invalid iterator
static int test_from_iterator_invalid(void) {
    DoublyLinkedList* new_list = dll_from_iterator(NULL, NULL);
    ...
    new_list = dll_from_iterator(&it, alloc);
    ASSERT_NULL(new_list);
...
typedef struct { int (*func)(void); const char *name; } TestCase;
TestCase tests[] = {
...
int main(void) {
    ...
    for (int i = 0; i < num_tests; i++) { if (tests[i].func() != TEST_SUCCESS) { printf("%s failed\n", tests[i].name); failed++; } }
    if (failed == 0) { printf("All DoublyLinkedList Iterator tests passed.\n"); return 0; }
