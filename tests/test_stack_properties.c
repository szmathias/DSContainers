...
int test_stack_lifo_property(void)
{
    ...
    for (int i = 0; i < num_elements; i++)
    {
        ...
        *values[i] = i * 7; // Use non-sequential values
        ASSERT_EQ(dsc_stack_push(stack, values[i]), 0);
    }
...
int test_stack_size_consistency(void)
{
    ...
    for (int i = 1; i <= 50; i++)
    {
        ...
        *data = i;
        ...
        ASSERT_EQ(dsc_stack_size(stack), (size_t)i);
        ASSERT(!dsc_stack_is_empty(stack));
    }
    ...
    for (int i = 49; i >= 0; i--)
    {
        ASSERT_EQ(dsc_stack_pop(stack, true), 0);
        ASSERT_EQ(dsc_stack_size(stack), (size_t)i);
...
int test_stack_peek_invariant(void)
{
    ...
    *data3 = 30;
    ...
    size_t original_size = dsc_stack_size(stack);

    // Multiple peeks should return same value and not change size
    for (int i = 0; i < 10; i++)
    {
        void* peeked = dsc_stack_peek(stack);
        ASSERT_NOT_NULL(peeked);
        ASSERT_EQ(*(int*)peeked, 30);
        ASSERT_EQ(dsc_stack_size(stack), original_size);

        // Also test top() alias
        void* top = dsc_stack_top(stack);
...
int test_stack_copy_preserves_order(void)
{
    ...
    // Create shallow copy
    DSCStack* shallow_copy = dsc_stack_copy(original);
    ASSERT_NOT_NULL(shallow_copy);

    // Create deep copy
    DSCStack* deep_copy = dsc_stack_copy_deep(original, false);
    ASSERT_NOT_NULL(deep_copy);

    // All three stacks should have same size and equal contents
    ASSERT_EQ(dsc_stack_size(original), (size_t)num_values);
    ASSERT_EQ(dsc_stack_size(shallow_copy), (size_t)num_values);
    ASSERT_EQ(dsc_stack_size(deep_copy), (size_t)num_values);

    ASSERT_EQ(dsc_stack_equals(original, shallow_copy, int_cmp), 1);
    ASSERT_EQ(dsc_stack_equals(original, deep_copy, int_cmp), 1);

    // Pop from all three - should get same sequence
    for (int i = num_values - 1; i >= 0; i--)
    {
        void* orig_data = dsc_stack_pop_data(original);
        void* shallow_data = dsc_stack_pop_data(shallow_copy);
        void* deep_data = dsc_stack_pop_data(deep_copy);
...
int test_stack_clear_preserves_structure(void)
{
    ...
    ASSERT_EQ(dsc_stack_size(stack), 10);
...
int test_stack_for_each_preserves_contents(void)
{
    ...
    dsc_stack_for_each(stack, increment);
