...
int test_stack_copy_shallow(void)
{
    ...
    ASSERT_EQ(dsc_stack_size(copy), 5);
    ASSERT_EQ(dsc_stack_equals(original, copy, int_cmp), 1);
    ...
    for (int i = 4; i >= 0; i--)
    {
        ...
        void* copy_data = dsc_stack_pop_data(copy);
...
int test_stack_copy_deep(void)
{
    ...
    // Create deep copy
    DSCStack* copy = dsc_stack_copy_deep(original, false);
    ASSERT_NOT_NULL(copy);
    ASSERT_EQ(dsc_stack_size(copy), 3);
    ASSERT_EQ(dsc_stack_equals(original, copy, int_cmp), 1);
...
int test_stack_for_each(void)
{
    ...
    dsc_stack_for_each(NULL, increment); // Should be safe
...
int test_stack_with_persons(void)
{
    ...
    ASSERT_EQ(dsc_stack_push(stack, bob), 0);
    ASSERT_EQ(dsc_stack_push(stack, charlie), 0);

    // Peek at top (should be Charlie)
    Person* top = (Person*)dsc_stack_peek(stack);
    ASSERT_NOT_NULL(top);
    ASSERT_EQ_STR(top->name, "Charlie");
    ASSERT_EQ(top->age, 35);

    // Test deep copy
    DSCStack* copy = dsc_stack_copy_deep(stack, false);
    ASSERT_NOT_NULL(copy);
    ASSERT_EQ(dsc_stack_equals(stack, copy, person_cmp), 1);
