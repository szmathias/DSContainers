...
int test_custom_allocator(void){
    DSCSinglyLinkedList *list = dsc_sll_create_custom(test_calloc, test_dealloc);
...
int test_copy_deep(void){
    ...
    // Create deep clone
    DSCSinglyLinkedList *clone = dsc_sll_copy_deep(list, int_copy, int_free);
    ASSERT_NOT_NULL(clone);
    ASSERT_EQ(clone->size, list->size);
    ...
    *first_value = 999;
    ...
    dsc_sll_destroy(list, int_free);
    dsc_sll_destroy(clone, int_free);
...
int test_copy_complex_data(void){
    ...
    // Create deep clone
    DSCSinglyLinkedList *clone = dsc_sll_copy_deep(list, person_copy, person_free);
    ASSERT_NOT_NULL(clone);
    ASSERT_EQ(clone->size, list->size);
    ...
    dsc_sll_destroy(clone, person_free);
...
int test_copy_empty(void){
    ...
    DSCSinglyLinkedList *deep_clone = dsc_sll_copy_deep(list, int_copy, int_free);
    ...
    // Cleanup
    dsc_sll_destroy(list, NULL);
    dsc_sll_destroy(shallow_clone, NULL);
    dsc_sll_destroy(deep_clone, NULL);
...
int test_copy_null(void){
    ...
    ASSERT_NULL(dsc_sll_copy_deep(NULL, int_copy, int_free));
...
int test_transform_allocation_failure(void) {
    ...
    DSCSinglyLinkedList *list = dsc_sll_create_custom(failing_alloc, failing_free);
    ASSERT_NOT_NULL(list);
    for (int i = 0; i < 5; i++) {
    ...
    DSCSinglyLinkedList *mapped1 = dsc_sll_transform(list, double_value_failing, failing_free);
    ASSERT_NULL(mapped1);
    ...
    DSCSinglyLinkedList *mapped2 = dsc_sll_transform(list, double_value_failing, failing_free);
    ...
    DSCSinglyLinkedList *mapped3 = dsc_sll_transform(list, double_value_failing, failing_free);
    ASSERT_NULL(mapped3);
...
int test_copy_deep_allocation_failure(void) {
    ...
    set_alloc_fail_countdown(0);
    DSCSinglyLinkedList *clone1 = dsc_sll_copy_deep(list, failing_int_copy, failing_free);
    ASSERT_NULL(clone1);

    // Case 2: Fail allocating a node partway through
    set_alloc_fail_countdown(3); // 1=clone list, 2=data0, 3=node0, FAIL on data1
    DSCSinglyLinkedList *clone2 = dsc_sll_copy_deep(list, failing_int_copy, failing_free);
    ASSERT_NULL(clone2);

    // Case 3: Fail allocating the *data* partway through
    set_alloc_fail_countdown(2); // 1=clone list, 2=data0, FAIL on node0
    DSCSinglyLinkedList *clone3 = dsc_sll_copy_deep(list, failing_int_copy, failing_free);
    ASSERT_NULL(clone3);
