 ...
 */
int dsc_sll_push_front(DSCSinglyLinkedList* list, void* data)
{
    if (!list)
    {
        return -1;
 ...
 */
int dsc_sll_push_back(DSCSinglyLinkedList* list, void* data)
{
    if (!list)
    {
        return -1;
...
int dsc_sll_insert_at(DSCSinglyLinkedList* list, const size_t pos, void* data)
{
    ...
    if (pos == 0)
    {
        return dsc_sll_push_front(list, data);
    }
 ...
 */
int dsc_sll_pop_front(DSCSinglyLinkedList* list, const bool should_free_data)
{
    if (!list || !list->head)
    {
        return -1;
 ...
 * Remove last element.
 */
int dsc_sll_pop_back(DSCSinglyLinkedList* list, const bool should_free_data)
{
    if (!list || !list->head)
    {
        return -1;
    ...
    if (list->size == 1)
    {
        return dsc_sll_pop_front(list, should_free_data);
    }
...
DSCSinglyLinkedList* dsc_sll_filter(const DSCSinglyLinkedList* list, const pred_func pred)
{
    ...
    while (curr)
    {
        if (pred(curr->data))
        {
            if (dsc_sll_push_back(filtered, curr->data) != 0)
            {
...
DSCSinglyLinkedList* dsc_sll_filter_deep(const DSCSinglyLinkedList* list, const pred_func pred)
{
    ...
    while (curr)
    {
        if (pred(curr->data))
        {
            ...
            if (dsc_sll_push_back(filtered, filtered_data) != 0)
            {
                if (filtered_data)
                {
...
DSCSinglyLinkedList* dsc_sll_transform(const DSCSinglyLinkedList* list, const transform_func transform, const bool should_free_data)
{
    ...
    while (curr)
    {
        ...
        if (dsc_sll_push_back(transformed, new_data) != 0)
        {
...
DSCSinglyLinkedList* dsc_sll_copy(const DSCSinglyLinkedList* list)
{
    ...
    while (curr)
    {
        if (dsc_sll_push_back(clone, curr->data) != 0)
        {
...
DSCSinglyLinkedList* dsc_sll_copy_deep(const DSCSinglyLinkedList* list, const copy_func copy_data, const bool should_free_data)
{
    ...
    while (curr)
    {
        ...
        if (dsc_sll_push_back(clone, data_copy) != 0)
        {
            if (should_free_data)
            {
...
DSCSinglyLinkedList* dsc_sll_from_iterator(DSCIterator* it, DSCAllocator* alloc, const bool should_copy)
{
    ...
    while (it->has_next(it))
    {
        ...
        if (dsc_sll_push_back(list, element_to_insert) != 0)
        {
            if (should_copy)
            {
