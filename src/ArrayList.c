...
static int ensure_capacity(DSCArrayList* list, const size_t min_capacity)
{
    ...
    void** new_data = dsc_alloc_malloc(list->alloc, new_capacity * sizeof(void*));
    if (!new_data)
    {
        return -1;
    }
...
DSCArrayList* dsc_arraylist_create(DSCAllocator* alloc, const size_t initial_capacity)
{
    ...
    DSCArrayList* list = dsc_alloc_malloc(alloc, sizeof(DSCArrayList));
    if (!list)
    {
        return NULL;
    }
...
void dsc_arraylist_destroy(DSCArrayList* list, const bool should_free_data)
{
    ...
    dsc_arraylist_clear(list, should_free_data);
...
void dsc_arraylist_clear(DSCArrayList* list, const bool should_free_data)
{
    ...
    if (should_free_data)
    {
        for (size_t i = 0; i < list->size; i++)
        {
            if (list->data[i])
            {
                dsc_alloc_data_free(list->alloc, list->data[i]);
            }
        }
    }
...
int dsc_arraylist_set(DSCArrayList* list, const size_t index, void* data, const bool should_free_old)
{
    ...
    {
        dsc_alloc_data_free(list->alloc, list->data[index]);
    }
...
void* dsc_arraylist_back(const DSCArrayList* list)
{
    ...
    return dsc_arraylist_get(list, list->size - 1);
}
...
int dsc_arraylist_remove(DSCArrayList* list, const void* data, const cmp_func compare, const bool should_free_data)
{
    ...
    return dsc_arraylist_remove_at(list, index, should_free_data);
}
...
int dsc_arraylist_shrink_to_fit(DSCArrayList* list)
{
    ...
    void** new_data = dsc_alloc_malloc(list->alloc, list->size * sizeof(void*));
    if (!new_data)
    {
        return -1;
    }
...
int dsc_arraylist_sort(DSCArrayList* list, const cmp_func compare)
{
    ...
    void** temp = dsc_alloc_malloc(list->alloc, list->size * sizeof(void*));
    if (!temp)
    {
        return -1;
    }
    ...
    dsc_alloc_free(list->alloc, temp);
...
DSCArrayList* dsc_arraylist_filter(const DSCArrayList* list, const pred_func pred)
{
    ...
    for (size_t i = 0; i < list->size; i++)
    {
        if (pred(list->data[i]))
        {
            if (dsc_arraylist_push_back(filtered, list->data[i]) != 0)
            {
...
DSCArrayList* dsc_arraylist_filter_deep(const DSCArrayList* list, const pred_func pred)
{
    ...
    for (size_t i = 0; i < list->size; i++)
    {
        if (pred(list->data[i]))
        {
            void* filtered_data = dsc_alloc_copy(filtered->alloc, list->data[i]);
...
DSCArrayList* dsc_arraylist_transform(const DSCArrayList* list, const transform_func transform, const bool should_free_data)
{
    ...
    for (size_t i = 0; i < list->size; i++)
    {
        ...
        if (dsc_arraylist_push_back(transformed, new_data) != 0)
        {
            ...
            dsc_arraylist_destroy(transformed, should_free_data);
            return NULL;
        }
    }
...
DSCArrayList* dsc_arraylist_copy(const DSCArrayList* list)
{
    ...
    for (size_t i = 0; i < list->size; i++)
    {
        if (dsc_arraylist_push_back(copy, list->data[i]) != 0)
        {
            dsc_arraylist_destroy(copy, false);
            return NULL;
        }
    }
...
DSCArrayList* dsc_arraylist_copy_deep(const DSCArrayList* list, const bool should_free_data)
{
    ...
    for (size_t i = 0; i < list->size; i++)
    {
        void* copied_data = dsc_alloc_copy(list->alloc, list->data[i]);
        if (dsc_arraylist_push_back(copy, copied_data) != 0)
        {
            dsc_alloc_data_free(copy->alloc, copied_data);
            dsc_arraylist_destroy(copy, should_free_data);
            return NULL;
        }
    }
